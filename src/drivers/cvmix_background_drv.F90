...
!BOC
    ...
    ! Two different columns are tested with each of two different memory
    ! options:
    ! Column 1 uses the internal background parameter dataset while column 2
    ! uses CVmix_BL_params; for the memory copy, column 1 tests the routine
    ! cvmix_get_bkgnd_real_2D() rather than cvmix_coeffs_bkgnd.

    ! Pointer test
    call cvmix_init_bkgnd(CVmix_vars_pointer(1), col1_vdc1, col1_vdc2,        &
                          col1_linv, col1_dpth, CVmix_params, CVMixLog)
    call cvmix_init_bkgnd(CVmix_vars_pointer(2), col2_vdc1, col2_vdc2,        &
                          col2_linv, col2_dpth, CVMix_params, CVMixLog,       &
